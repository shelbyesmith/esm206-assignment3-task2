...
sw_split_name<- starwars %>% 
  separate(col = name,
           ...
           sep="-")
# Additional pieces discarded in 1 rows [49]
```
...
#This keeps the column name but then adds 2 columns, one is the first name and the second is the rest of the name. If there are more than 2 names, the extra pieces all get put into the second new column. 
```
...
```{r}
# Your code here: 
mpg_model_class<-mpg %>% #starting from the `mpg` data create a new data frame called `mpg_model_class`
  unite(col=model_class, #new column called `model_class`
        c(model, class), #combines the `model` and `class` column information
        sep= "-", #separated by a dash "-"
        remove=FALSE) #retains the original columns
...
```{r}
# Your code here: 
some_dates<- c(08031994,08151954, 11161953) #create a vector (using `c()`, as above) stored as `some_dates` that contains three values that represent dates as numbers, in mdy format: 11061972, 02142007,01301843
some_dates_converted<-mdy(some_dates) #use `lubridate::mdy()` to return them in date format, storing as a vector `some_dates_converted`
class(some_dates_converted) #check the class of that vector
```
...
lakers_dates <- lakers %>% 
  mutate(date_new = ymd(date)) %>% #add a `date_new` column with the nicely converted date
...
gasol_hookshots<- lakers %>% 
  filter(player== "Pau Gasol", type== "hook") %>% #Filter to only include player is Pau Gasol and type (shot type) is "hook"
  mutate(date=ymd(date)) %>% #Use mutate() to update (or add (would have named it something other than date) the date column to "Date" class with `ymd()`
  mutate(month=month(date)) #Add a new column that contains the month, parsed from the date created above with `month()`

hookshots_by_month<- gasol_hookshots %>% 
  count(month) #count() function combines group_by + summarize + n all in one

view(hookshots_by_month)

class(gasol_hookshots$date) #check class with gasol_hookshots$date because just using class(date) refers to the date funciton...
class(gasol_hookshots$month) #check class...it's numeric 
```
...
storms_danger<- storms %>% 
  mutate(danger=case_when(                   #add a new column `danger` where if
    status == "hurricane" ~ "high",          #status matches "hurricane", then the danger column contains "high"
    status != "hurricane" ~ "moderate"))     #if the status is anything other than "hurricane", then the danger column contains "moderate"
